...
2. **CMake + Ninja**: `choco install cmake ninja -y`
...
The `choco` CLI itself must also be installed — the app is a GUI wrapper around it.
...
- `src/` — the choco adapter (command builders + output parsers) and `main.cpp`.
